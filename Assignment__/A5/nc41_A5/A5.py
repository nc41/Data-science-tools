wfrom pyspark import SparkContext
...
def LogTraining(Loss_Diff, lr, regulization, epoch, verbose=True):
    # w = 0.05 * np.random.sample((lengthOfdic,)) - 0.05
    ...
    for i in range(epoch):
        ...
        H = TrainLabel.map(lambda x: (
            x[0], (x[1][0], x[1][1], np.dot(x[1][0], w))))
        c = H.map(lambda x: (
            x[0], (-x[1][1] * x[1][2] + math.log(1 + np.exp(x[1][2])))))
        l = (1.0 / docNum) * (c.reduce(lambda a, b: ('', a[1] + b[1]))[1]) + (
            regulization / (2.0 * docNum)) * np.sum(np.square(w))
        gradient = H.map(lambda x: (x[0], -x[1][0] * x[1][1] + x[1][0] * (np.exp(
            x[1][2]) / (1.0 + np.exp(x[1][2]))))).reduce(lambda a, b: ('', a[1] + b[1]))[1]
...
weight = LogTraining(0.00005, 0.5, 0, 300, True)
